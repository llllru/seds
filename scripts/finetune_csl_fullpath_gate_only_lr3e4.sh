...
VISIBLE_GPUS="${CUDA_VISIBLE_DEVICES:-2,3}"
DIST_PORT="${MASTER_PORT:-29460}"
...
CUDA_VISIBLE_DEVICES="${VISIBLE_GPUS}" python -m torch.distributed.launch \
  ...
  --output_dir result_train/csl_posergbfusion_fullpath_gate_only_lr3e4 \
  --init_model "/media/hdd2/lyr2025/seds_data/csl_best_model.bin" \
  --signbert \
  ...
  --batch_size_val 64 \
  --datatype csl_pose --coef_lr 1.0 --freeze_layer_num 0 \
